...
} from "@/redux/allHcaCircuitTournamentSlice";
import HcaCircuitTournamentsList from "./HcaCircuitTournamentsList";
...
const HcaCircutComponent = () => {
  ...
  const [
    filtereHcaCircuitTournamentCount,
    setfiltereHcaCircuitTournamentCount,
  ] = useState(0);
  const [currentPage, setCurrentPage] = useState(1); // Current page number
  const [hcaCircuitTournamentsPerPage] = useState(7);
  ...
  const startItem = (currentPage - 1) * hcaCircuitTournamentsPerPage + 1;
  const endItem = Math.min(
    currentPage * hcaCircuitTournamentsPerPage,
    filtereHcaCircuitTournamentCount
  );
  const showingText = `Showing ${startItem}-${endItem} of ${filtereHcaCircuitTournamentCount}`;
  ...
  // filter
  useEffect(() => {
    // filter hca circuit tournaments
    let tempFilteredHcaCircuitTournamentsList =
      allActiveHcaCircuitTournamentsList?.slice();

    // filter by branch
    tempFilteredHcaCircuitTournamentsList =
      selectedBranch?.toLowerCase() == "all"
        ? tempFilteredHcaCircuitTournamentsList
        : tempFilteredHcaCircuitTournamentsList?.filter(
            (tournament: any) =>
    ...
    if (useAdvancedDate) {
      tempFilteredHcaCircuitTournamentsList =
        tempFilteredHcaCircuitTournamentsList?.filter((tournament: any) => {
    ...
    tempFilteredHcaCircuitTournamentsList =
      tempFilteredHcaCircuitTournamentsList?.sort(
        (a: any, b: any) =>
    ...
    setfiltereHcaCircuitTournamentCount(
      tempFilteredHcaCircuitTournamentsList?.length
    );
    setCurrentPage(1);
    dispatch(
      filterHcaCircuitTournamentList(tempFilteredHcaCircuitTournamentsList)
    );
  }, [
    allActiveHcaCircuitTournamentsList,
    selectedBranch,
  ...
  // intial data fetch
  useEffect(() => {
    dispatch(fetchAllHcaCircuitTournaments());
    ...
    <div className="flex-1 flex flex-col py-6 px-10 border bg-white rounded-lg">
      <div className="main-header flex justify-between">
        <div className="title-menus flex items-center gap-4 ">
          ...
          <h2 className="text-3xl mb-2 font-medium text-gray-700 flex items-center">
            <Trophy />
            <span className="ml-1">HCA Circuit Tournaments </span>
          </h2>
        </div>
      ...
      <div className="batches-header my-0 flex items-end justify-between gap-2">
        ...
        <div className="search-filter-menus flex gap-4">
          ...
          "trainer" ? (
            <Link
              href={`/${session?.data?.user?.role?.toLowerCase()}/tournaments/hcacircuit/addhcacircuittournament`}
            >
      ...
      <HcaCircuitTournamentsList
        allFilteredActiveHcaCircuitTournamentsList={
          allFilteredActiveHcaCircuitTournamentsList
        }
        currentPage={currentPage}
        hcaCircuitTournamentsPerPage={hcaCircuitTournamentsPerPage}
        allHcaCircuitTournamentsLoading={allHcaCircuitTournamentsLoading}
      />

      <Stack spacing={2} className="mx-auto w-max mt-3">
        <Pagination
          count={Math.ceil(
            filtereHcaCircuitTournamentCount / hcaCircuitTournamentsPerPage
          )} // Total pages
...
export default HcaCircutComponent;
