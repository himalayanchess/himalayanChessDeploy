...
const UpdateHcaCircuitTournament = ({ hcaCircuitTournamentRecord }: any) => {
  ...
  const onSubmit = async (data: any) => {
    setLoading(true);
    try {
      //   console.log("update hca circuit tournamentdata", data);
      const { data: response } = await axios.post(
        "/api/tournaments/hcacircuit/updatehcacircuittournament",
        {
          ...data,
        }
      );
  ...
  useEffect(() => {
    if (hcaCircuitTournamentRecord) {
      reset({
        ...
        startDate: hcaCircuitTournamentRecord.startDate
          ? dayjs(hcaCircuitTournamentRecord.startDate)
              .tz(timeZone)
              .format("YYYY-MM-DD")
          : "",
        endDate: hcaCircuitTournamentRecord.endDate
          ? dayjs(hcaCircuitTournamentRecord.endDate)
              .tz(timeZone)
              .format("YYYY-MM-DD")
          : "",
      });

      setValue("branchName", hcaCircuitTournamentRecord?.branchName);
      setValue("branchId", hcaCircuitTournamentRecord?.branchId);

      setselectedBranch(hcaCircuitTournamentRecord?.branchName);
      setLoaded(true);
    }
  }, [hcaCircuitTournamentRecord, reset]);
        ...
        <div className="heading flex items-center gap-4">
          <div className="header w-full flex items-end justify-between">
            <h1 className="w-max mr-auto text-2xl flex items-center font-bold">
              <Trophy />
              <span className="ml-2">Update HCA Circuit Tournament</span>
            </h1>
...
export default UpdateHcaCircuitTournament;
