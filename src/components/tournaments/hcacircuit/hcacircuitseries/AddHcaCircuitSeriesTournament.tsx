...
const AddHcaCircuitSeriesTournament = ({
  ...
  const onSubmit = async (data: any) => {
    setLoading(true);
    try {
      console.log("add hca circuit series tournamentdata", data);
      const { data: response } = await axios.post(
        "/api/tournaments/hcacircuit/hcacircuitseries/addhcacircuitseriestournament",
        {
          ...data,
        }
      );
        ...
        setTimeout(() => {
          router.push(
            `/${session?.data?.user?.role?.toLowerCase()}/tournaments/hcacircuit/${
              mainHcaCircuitTournamentRecord?._id
            }`
          );
        }, 50);
      } else {
  ...
  // update mainHcaCircuitTournamentId and name after mainHcaCircuitTournamentRecord load
  useEffect(() => {
    if (mainHcaCircuitTournamentRecord) {
      setValue(
        "mainHcaCircuitTournamentId",
        mainHcaCircuitTournamentRecord?._id
      );
      setValue(
        "mainHcaCircuitTournamentName",
        mainHcaCircuitTournamentRecord?.tournamentName
      );
    }
  }, [mainHcaCircuitTournamentRecord]);
  ...
  useEffect(() => {
    ...
    // replace this logic in update hca series tournament
    resetParticipant([]);
          ...
          <div className="header w-full flex items-start justify-between">
            <div className="title-tournamentname flex flex-col">
              <h1 className="w-max mr-auto text-2xl flex items-center font-bold">
                <Trophy />
                <span className="ml-2">Add HCA Circuit Series Tournament</span>
              </h1>
              <span className="text-md">
                of {mainHcaCircuitTournamentRecord?.tournamentName} (
                {mainHcaCircuitTournamentRecord?.branchName})
              </span>
            </div>
...
export default AddHcaCircuitSeriesTournament;
