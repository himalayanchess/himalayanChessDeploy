...
const UpdateHcaCircuitSeriesTournament = ({
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
        ...
        setTimeout(() => {
          router.push(
            `/${session?.data?.user?.role?.toLowerCase()}/tournaments/hcacircuit/${
              hcaCircuitSeriesTournamentRecord?.mainHcaCircuitTournamentId
            }`
          );
        }, 50);
      } else {
      ...
      console.error("Error submitting hca circuit series  tournament:", error);
      notify("Failed to submit hca circuit series tournament", 500);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (hcaCircuitSeriesTournamentRecord) {
      reset({
        ...hcaCircuitSeriesTournamentRecord,
        startDate: hcaCircuitSeriesTournamentRecord.startDate
          ? dayjs(hcaCircuitSeriesTournamentRecord.startDate)
              .tz(timeZone)
              .format("YYYY-MM-DD")
          : "",
        endDate: hcaCircuitSeriesTournamentRecord.endDate
          ? dayjs(hcaCircuitSeriesTournamentRecord.endDate)
              .tz(timeZone)
              .format("YYYY-MM-DD")
          : "",
        participants: hcaCircuitSeriesTournamentRecord?.participants?.filter(
      ...
      setValue("branchName", hcaCircuitSeriesTournamentRecord?.branchName);
      setValue("branchId", hcaCircuitSeriesTournamentRecord?.branchId);

      setselectedBranch(hcaCircuitSeriesTournamentRecord?.branchName);
      setLoaded(true);
    }
  }, [hcaCircuitSeriesTournamentRecord, reset]);
    ...
    <div className="flex w-full h-full">
      <div className="flex-1 flex w-full flex-col h-full overflow-hidden bg-white px-10 py-5 rounded-md shadow-md">
        <div className="heading flex items-center gap-4">
          <div className="header w-full flex items-start justify-between">
            <div className="title-tournamentname flex flex-col">
              <h1 className="w-max mr-auto text-2xl flex items-center font-bold">
                <Trophy />
                <span className="ml-2">
                  Update HCA Circuit Series Tournament
                </span>
              </h1>
              <span className="text-md">
                of {hcaCircuitSeriesTournamentRecord?.tournamentName} (
            ...
            {participantsFields.map((field, index) => (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="rank-points grid grid-cols-2 gap-4">
                    {/* rank */}
                    <Controller
...
export default UpdateHcaCircuitSeriesTournament;
