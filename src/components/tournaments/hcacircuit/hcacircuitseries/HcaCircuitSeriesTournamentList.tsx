...
import { deleteMainHcaCircuitSeriesTournament } from "@/redux/allHcaCircuitTournamentSlice";
...
const HcaCircuitSeriesTournamentList = ({ tournamentList, loading }: any) => {
  ...
  const [
    selectedhcaCircuitSeriesTournamentId,
    setSelectedhcaCircuitSeriesTournamentId,
  ] = useState(null);
  const [
    selectedhcaCircuitSeriesTournamentName,
    setSelectedhcaCircuitSeriesTournamentName,
  ] = useState("");
  // Delete Modal
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);

  function handleDeleteModalOpen(
    hcaCircuitSeriesTournamentId: any,
    hcaCircuitSeriesTournamentName: any
  ) {
    setSelectedhcaCircuitSeriesTournamentId(hcaCircuitSeriesTournamentId);
    setSelectedhcaCircuitSeriesTournamentName(hcaCircuitSeriesTournamentName);
    setDeleteModalOpen(true);
  }
  function handleDeleteModalClose() {
    setDeleteModalOpen(false);
  }
  // handlehcaCircuitSeriesTournamentDelete
  async function handlehcaCircuitSeriesTournamentDelete(id: any) {
    try {
      const { data: resData } = await axios.post(
        "/api/tournaments/hcacircuit/deletehcacircuitseriestournament",
        {
          hcaCircuitSeriesTournamentId: id,
        }
      );
      if (resData.statusCode == 200) {
        notify(resData.msg, resData.statusCode);
        dispatch(deleteMainHcaCircuitSeriesTournament(id));
  ...
  return (
    <div className="overflow-y-auto mt-3 h-full flex-1 border flex flex-col bg-white rounded-lg">
      {/* Table Headings */}
      ...
      {/* No hcaCircuitSeriesTournament Found */}
      ...
      {/* hcaCircuitSeriesTournament List */}
      {!loading && (
        <div className="table-contents overflow-y-auto h-full  flex-1 grid grid-cols-1 grid-rows-7">
          {tournamentList.map((hcaCircuitSeriesTournament: any, index: any) => {
            const serialNumber = index + 1;
            return (
              <div
                key={hcaCircuitSeriesTournament?._id}
              ...
              >
                ...
                {/* hcaCircuitSeriesTournamentname */}
                <Link
                  title="View"
                  href={`/${session?.data?.user?.role?.toLowerCase()}/tournaments/hcacircuit/hcacricuitseriestournament/${
                    hcaCircuitSeriesTournament?._id
                  }`}
                  className="text-left text-sm col-span-2 font-medium text-gray-600 hover:underline hover:text-blue-500"
                >
                  {hcaCircuitSeriesTournament?.tournamentName}
                </Link>
                {/* date */}
                <span className="  text-sm text-gray-700">
                  {hcaCircuitSeriesTournament?.startDate
                    ? dayjs(hcaCircuitSeriesTournament?.startDate)
                        .tz(timeZone)
                        .format("MMMM D, YYYY")
                    : "N/A"}
                </span>
                {/* week no */}
                <span className=" text-sm text-gray-700">
                  {hcaCircuitSeriesTournament?.totalRounds
                    ? `${hcaCircuitSeriesTournament?.totalRounds}`
                    : "N/A"}
                </span>
                {/* tournament type */}
                <span className=" text-sm text-gray-700 flex items-center">
                  {hcaCircuitSeriesTournament?.tournamentType?.toLowerCase() ===
                  "standard" ? (
                  ...
                  <span className="ml-1">
                    {hcaCircuitSeriesTournament?.tournamentType}
                  </span>
                </span>
                {/* branch name */}
                <span className=" col-span-1 text-sm text-gray-700">
                  {hcaCircuitSeriesTournament.branchName
                    ? hcaCircuitSeriesTournament.branchName
                    : "N/A"}
                </span>
                {session?.data?.user?.role?.toLowerCase() != "trainer" ? (
                  <div className=" text-sm text-gray-500">
                    <>
                      {/* edit */}
                      <Link
                        href={`/${session?.data?.user?.role?.toLowerCase()}/tournaments/hcacircuit/updatehcacircuitseriestournament/${
                          hcaCircuitSeriesTournament?._id
                        }`}
                        title="Edit"
                      ...
                      {/* delete modal */}
                      {hcaCircuitSeriesTournament?.activeStatus == true && (
                        <button
                          title="Delete"
...
export default HcaCircuitSeriesTournamentList;
