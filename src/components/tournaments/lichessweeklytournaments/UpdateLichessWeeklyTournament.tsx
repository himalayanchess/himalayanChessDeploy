...
const UpdateLichessWeeklyTournament = ({ lichessTournamentRecord }: any) => {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      notify("Failed to update lichess tournament", 500);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (lichessTournamentRecord) {
      reset({
        ...
        date: lichessTournamentRecord.date
          ? dayjs(lichessTournamentRecord.date)
              .tz(timeZone)
              .format("YYYY-MM-DD")
          : "",
        time: lichessTournamentRecord.time
          ? dayjs(lichessTournamentRecord.time).tz(timeZone).toISOString()
          : null,
        lichessWeeklyWinners:
          lichessTournamentRecord?.lichessWeeklyWinners?.filter(
            (winner: any) => winner.activeStatus
          ),
      });

      // clock time
      setValue("initialTime", lichessTournamentRecord?.clockTime?.initialTime);
      setValue("increment", lichessTournamentRecord?.clockTime?.increment);

      setValue("branchName", lichessTournamentRecord?.branchName);
      setValue("branchId", lichessTournamentRecord?.branchId);

      setselectedBranch(lichessTournamentRecord?.branchName);
      setLoaded(true);
    }
  }, [lichessTournamentRecord, reset]);
        ...
        <div className="heading flex items-center gap-4">
          <div className="header w-full flex items-end justify-between">
            <h1 className="w-max mr-auto text-2xl flex items-center font-bold">
              <Trophy />
              <span className="ml-2">Update Lichess Tournament</span>
            </h1>
...
export default UpdateLichessWeeklyTournament;
